function buscar(){msg=[];let e={},t=new Date(Date.now()),a=["fetch.php?f=fornecedor","fetch.php?f=operacoes","fetch.php?f=baixadas","fetch.php?f=taxas",`fetch.php?f=feriados&data=${t.getFullYear()}-${t.getMonth()+1}-${t.getDate()}`],o=new Intl.NumberFormat("pt-BR",{style:"currency",currency:"BRL",minimumFractionDigits:2}),n=0,s=0,r=0,d=0;function l(){n=0,s=0,r=0,d=0,txBoleto=0,txTed=0,txTac=0,e.fornecedor.operacoes.forEach(e=>{console.log("OVER OP.VALOR",e.valor),console.log("typeof op.valor",typeof e.valor),!0===e.antecipar&&(n+=new Number(e.valor),s+=new Number(e.valorDesconto),d++)}),txBoleto=new Number(0===e.fornecedor.boleto?e.taxas.find(e=>"BOLETO"===e.titulo).valor:e.fornecedor.boleto),txTed=new Number(0===parseFloat(e.fornecedor.ted)?e.taxas.find(e=>"TED"===e.titulo).valor:e.fornecedor.ted),txTac=new Number(0===parseFloat(e.fornecedor.tac)?e.taxas.find(e=>"TAC"===e.titulo).valor:e.fornecedor.tac),console.log("forneedor TAC",e.fornecedor.tac),console.log("forneedor TED",e.fornecedor.tac),console.log("TX BOLETO",e.fornecedor.tac),r+=new Number(new Number(txBoleto*d+txTed+txTac).toFixed(2)),console.log("valor taxas is",r);let a=document.querySelector("#footer-operacoes"),l=document.createElement("tr"),c="";console.log("fornecedor is >>>",e.fornecedor),c=`
			...
			<td>
				...
				<span class="text-dark fw-bolder d-block fs-5">${o.format(new Number(n).toFixed(2))}</span>
			</td>
			<td>
			...
			<td>
				...
				<span class="text-dark fw-bolder d-block fs-5">${o.format(new Number(s).toFixed(2))}</span>
			</td>
			<td>
				<span class="text-muted fw-bold d-block fs-7">desconto taxas</span>
				<span class="text-dark fw-bolder d-block fs-5">${o.format(new Number(r).toFixed(2))}</span>
			</td>
			<td>
				<span class="text-muted fw-bold d-block fs-7">valor antecipa\xe7\xe3o</span>
				<span class="text-dark fw-bolder d-block fs-5">${o.format(new Number(n-(s+r)).toFixed(2))}*</span>
			</td>
		...
		`,a.appendChild(l);let i=null;d>0&&document.querySelector("#efetuar-antecipadas").addEventListener("click",()=>{let a=[parseFloat((30/d).toFixed(2)),25,45],o=document.querySelector("#antecipadas-efetuando"),l=document.querySelector("#antecipadas-porcentagem"),c=document.querySelector("#antecipadas-progressbar"),p=document.querySelector("#antecipadas-modal-msg"),f=document.querySelector("#antecipadas-titulo"),b=0;exibeModal();let $={metodo:"antecipadas",fornecedor:e.fornecedor.id,data:t.toISOString().split("T")[0],valorOriginal:n,descontoJuros:s,descontoTaxas:r,valor:n-(s+r),status:0};fetch("fetch.php",{method:"POST",headers:{Accept:"application/json, text/plain, */*","Content-Type":"application/json"},body:JSON.stringify($)}).then(e=>e.text()).then(n=>{i=parseInt(n,10);let s=e.fornecedor.operacoes.filter(e=>!0===e.antecipar),r=[];s.forEach(e=>{let a={metodo:"detalhes",antecipada:i,operacao:e.id,nota:e.nota,data:t.toISOString().split("T")[0],valorOriginal:parseFloat(e.valor),descontoJuros:e.valorDesconto,valor:e.valor-e.valorDesconto};r.push(a)}),Promise.all(r.map(e=>{o.innerHTML="Efetuando antecipa\xe7\xe3o nota fiscal "+e.nota+"…",l.innerHTML=b+"%",fetch("fetch.php",{method:"POST",headers:{Accept:"application/json, text/plain, */*","Content-Type":"application/json"},body:JSON.stringify(e)}).then(e=>e.text()).then(t=>{b+=a[0],l.innerHTML=b+"%",c.setAttribute("aria-valuenow",b),c.style.width=b+"%",p.innerHTML+="✔ Opera\xe7\xe3o com nota fiscal "+e.nota+" antecipada.<br>"})})).then(()=>{setTimeout(function(){o.innerHTML="Redigindo contrato para a opera\xe7\xe3o "+i+"…",fetch("contrato.php",{method:"POST",headers:{Accept:"application/json, text/plain, */*","Content-Type":"application/json"},body:JSON.stringify({id:i})}).then(e=>e.text()).then(e=>{!0===(e=JSON.parse(e)).criado?(b+=a[1],l.innerHTML=b+"%",c.setAttribute("aria-valuenow",b),c.style.width=b+"%",p.innerHTML+='✔ Contrato criado: <a target="_blank" href="'+e.nome+'">'+e.nome+"</a>.<br>",o.innerHTML="Enviando contrato para assinaturas…",fetch("autentique.php",{method:"POST",headers:{Accept:"application/json, text/plain, */*","Content-Type":"application/json"},body:JSON.stringify({emailEmpresa:e.emailEmpresa,arquivo:e.nome,id_operacao:e.id_operacao})}).then(e=>e.text()).then(e=>{e=JSON.parse(e),console.log(e),Array.isArray(e)?(b+=a[2],o.innerHTML="",l.innerHTML=b+"%",c.setAttribute("aria-valuenow",b),c.style.width=b+"%",p.innerHTML+="✔ Contrato enviado para assinaturas.",f.innerHTML="Antecipa\xe7\xe3o Conclu\xedda!"):(o.innerHTML="",l.innerHTML="100%",c.setAttribute("aria-valuenow",100),c.style.width="100%",p.innerHTML+="✖ Erro ao enviar contrato para assinatura. Erro: "+e,f.innerHTML="Antecipa\xe7\xe3o Conclu\xedda!")})):(o.innerHTML="",l.innerHTML="100%",c.setAttribute("aria-valuenow",100),c.style.width="100%",p.innerHTML+="✖ Erro ao gerar contrato para opera\xe7\xe3o "+i+".",f.innerHTML="Antecipa\xe7\xe3o Conclu\xedda!")})},1050)})})})}Promise.all(a.map(e=>fetch(e))).then(e=>Promise.all(e.map(e=>e.json()))).then(t=>{e.fornecedor=t[0],e.fornecedor.operacoes=t[1],e.fornecedor.baixadas=t[2],e.taxas=t[3],e.feriado=t[4]}).finally(()=>(function a(){document.querySelector("#operacao-data").innerHTML=new Date(Date.now()).toLocaleDateString("pt-BR"),document.querySelector("#fornecedor").innerHTML=e.fornecedor.razao;let n=document.querySelector("#table-operacoes");n.innerHTML="";let s=e.fornecedor.limite,r=0,d=0;e.fornecedor.operacoes.length>0?e.fornecedor.operacoes.forEach((a,s)=>{r+=parseFloat(a.valor);let d=a.nota.split("/"),l=new Date(a.vencimento),c=0===parseFloat(e.fornecedor.juros)?2.5:parseFloat(e.fornecedor.juros),i=new Date(t.getFullYear(),t.getMonth()+1,0).getDate(),p=l.getDay(),f=1;switch(p){case 0:f+=2;break;case 5:case 6:f+=3}let b=Math.floor((l-t)/864e5)+f;a.dias=b,a.jurosDia=parseFloat(c/i).toFixed(2),a.valorDesconto=parseFloat(parseFloat(a.valor)*(a.jurosDia/100)*b).toFixed(2);let $=document.createElement("tr");$.innerHTML=`
				...
				`,n.appendChild($)}):(n.innerHTML="",l()),e.fornecedor.baixadas.length>0&&e.fornecedor.baixadas.forEach((e,t)=>{d+=parseFloat(e.valor)}),document.querySelector("#limiteDisponivel").innerHTML=o.format(s-d),document.querySelector("#disponivel").innerHTML=o.format(r),document.querySelector("#limiteTomado").innerHTML=o.format(d),function t(){let a=document.querySelectorAll(".operacoes-check");a.forEach(t=>{t.addEventListener("click",()=>{let a=t.getAttribute("data-attr");e.fornecedor.operacoes[a].antecipar=!!t.checked,l()})})}()})()).catch(()=>console.log("erro"))}function exibeModal(){let e=document.querySelector("#trigger-modal"),t=document.querySelectorAll(".fecha-modal");t.forEach(e=>e.addEventListener("click",()=>buscar())),e.click()}buscar();
