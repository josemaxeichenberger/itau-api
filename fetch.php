	...
	if ($_GET) {
		...
		switch($_GET['f']) {
			...
			case 'operacoes':
				...
				$hoje_mais = date("Y-m-d", strtotime($hoje . " +365 days"));
				$sql = "SELECT * FROM operacoes WHERE cnpj=? AND status=? AND vencimento>=? AND vencimento <=? ORDER BY vencimento ASC";
				$stmt = prepared_query($con, $sql, [$cnpj, 0, $hoje_mais_10_dias,$hoje_mais], 'siss');
