...
function buscar() {
	...
	function observaCheckboxes() {
		...
		let checkedCount = 0; // Contador para controlar o número de checkboxes marcados
	
		checkboxes.forEach(c => {
			c.addEventListener('click', () => {
				if (c.checked) {
					if (checkedCount >= 10) { // Verifica se já existem 10 checkboxes marcados
						c.checked = false; // Desmarca o checkbox se já houver 10 marcados
						Swal.fire({
							icon: "error",
							title: "Oops...",
							confirmButtonText: "OK",
							text: "Seu Limite é de 10 Operações por vez.!",
						  }).then((result) => {
							/* Read more about isConfirmed, isDenied below */
							if (result.isConfirmed) {
							} 
						});
						return; // Sai da função para evitar mais ações
					}
					checkedCount++; // Incrementa o contador se o checkbox for marcado
				} else {
					checkedCount--; // Decrementa o contador se o checkbox for desmarcado
				}
	
				const index = c.getAttribute('data-attr');
				if (c.checked) {
					obj.fornecedor.postergadas.push(index);
				} else {
					obj.fornecedor.postergadas.splice(obj.fornecedor.postergadas.indexOf(index), 1);
				}
				obj.fornecedor.operacoes[index].antecipar = c.checked;
				atualizaAntecipacao();
			});
		});
	}
